...
export class PedidosService {
    ...
    async cambiarEstado(id: string, nuevoEstado: string) {
        ...
        if (!permitidas.includes(nuevoEstado))
            throw new BadRequestException(`Transición: ${pedido.estadoPedido} -> ${nuevoEstado} no esta permitida`);


        return this.pedidoModel.findByIdAndUpdate(id, { estadoPedido: nuevoEstado }, { new: true }).exec();
    }
    ...
    async create(dto: { mesaId?: string, tipoPedido: string, estadoPedido?: string }) {
    ...
    async addItem(pedidoId: string, dto: { platoId: string, cantidad: number }) {
        ...
        if (pedido.estadoPedido !== 'pendiente') throw new BadRequestException('Solo se pueden agregar items a pedidos pendientes');
        ...
        pedido.total = pedido.items.reduce((sum, item) => sum + item.cantidad * item.precioUnitario, 0);
